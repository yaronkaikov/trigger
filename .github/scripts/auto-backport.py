...
def create_pr_comment_and_add_backport_error_label(pr, comment_body):
    pr.add_to_labels("backport-error")
...
def with_github_keyword_prefix(repo, pr):
    ...
    if not match:
        ...
        create_pr_comment_and_add_backport_error_label(pr, comment)
        return False
    else:
        return True
...
def main():
    ...
    for pr in closed_prs:
        ...
        # if 'backport-error' not in pr.get_labels():
